    ...
    if( !function_exists('aventura_setup') ):
        function aventura_setup(){
            ...
            register_nav_menus( array(
                ...
                'custom-menu-4' => esc_html__( 'Menu Left', 'aventura' ),
                'custom-menu-5' => esc_html__( 'Menu Right', 'aventura' ),
                'custom-menu-6' => esc_html__( 'Menu Shop', 'aventura' ),
                'custom-menu-7' => esc_html__( 'Menu Home Slide', 'aventura' ),
            ) );
...
//Front-End Styles
    function aventura_front_end_styles(){
        ...
        wp_enqueue_style('font-awesome5', get_template_directory_uri().'/css/all.css', false );
        ...
        wp_enqueue_style('font-proximanova', get_template_directory_uri().'/css/proximanova.css', false );
        ...
        if( is_singular('tour')){
            ...
            wp_enqueue_style('aventura-galleryowl', get_template_directory_uri().'/css/owl.carousel.min.css', false );
            wp_enqueue_script('aventura-galleryowljs', get_template_directory_uri() . '/js/owl.carousel.min.js', array(), false, true);
        }
        ...
        //update style
        wp_enqueue_style('aventura-update-css', get_template_directory_uri() . '/css/update_main.css', false);
        wp_register_style( 'aventura-style-inline', false,false );
    }
    ...
    */
        ...
        function aventura_breadcrumbs_custom() {
            ...
            if ( !is_front_page() ) {
                ...
                if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
                    ...
                    if($aventura_post_type != 'post' && $aventura_post_type !=false) {
                    ...
                    if(empty($aventura_last_category) && !empty($aventura_custom_taxonomy_1) && $aventura_taxonomy_exists_1) {
                        ...
                        if(isset($aventura_taxonomy_terms_1) && !empty($aventura_taxonomy_terms_1)){
    ...
    if ( ! function_exists( 'aventura_logo_url' ) ) {
        function aventura_logo_url() {
            ...
            if ( ! empty( $aventura_options['aventura_logo_images'] ) && ! empty( $aventura_options['aventura_logo_images']['url'] ) ) {
                ...
                $aventura_url = get_template_directory_uri() . '/images/logo.png';
            }
            return $aventura_url;
        }
    }
    ...
    if ( ! function_exists( 'aventura_get_all_comments_of_post_type' ) ) {
        function aventura_get_all_comments_of_post_type($aventura_post_type){
            global $wpdb;
            $aventura_comments = $wpdb->get_var("SELECT COUNT(comment_ID) FROM $wpdb->comments
        WHERE comment_post_ID in ( SELECT ID FROM $wpdb->posts WHERE post_type = '$aventura_post_type' AND post_status = 'publish')
...
//        $redirect_url = get_bloginfo('url') . '/register';
    ...
    function aventura_parent_comment_counter($post_id = 0 ){
        ...
        return apply_filters( 'aventura_parent_comment_counter', $count, $post_id );
    };
    
    /*method activie plugin*/
    require get_template_directory() . '/plugins/class-tgm-plugin-activation.php';
    
    add_filter( 'plazart-installation_register', 'tz_aventura_register_required_demos' );


if(phpversion() >=8){
    require get_template_directory() . '/functions-php8.php';
}else{
    require get_template_directory() . '/functions-install.php';
}
    ...
    function aventura_register_required_plugins()
    {
         ...
         */
        $tz_aventura_plugins = array(
            
            /* This is an example of how to include a plugin pre-packaged with a theme */
            array(
                'name' => esc_html__('Plazart installation', 'aventura'), /* The plugin name */
        ...
        tgmpa($tz_aventura_plugins, $tz_aventura_config);
    }
...
function aventura_excerpt($limit) {
...
function aventura_content($limit) {
...
// Add setting
add_action( 'admin_init', 'aventura_slug_setting' );
function aventura_slug_setting() {
    add_settings_field('aventura_tour_slug', __('Tour archive slug', 'aventura'), 'aventura_slug_output', 'permalink', 'optional');
}

// Setting output
function aventura_slug_output() {
    ?>
    <input name="aventura_tour_slug" type="text" class="regular-text code" value="<?php echo esc_attr(get_option('aventura_tour_slug')); ?>" placeholder="<?php echo 'tour'; ?>" />
    <?php
}

// Save setting
add_action( 'admin_init', 'aventura_slug_save_setting' );
function aventura_slug_save_setting() {
    if (isset($_POST['permalink_structure'])) {
        update_option('aventura_tour_slug', trim($_POST['aventura_tour_slug']));
    }
}
?>
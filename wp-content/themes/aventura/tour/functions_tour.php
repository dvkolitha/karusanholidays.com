...
if ( ! function_exists( 'aventura_price' ) ) {
    function aventura_price( $aventura_amount, $aventura_type="" ) {
        ...
        if ($aventura_decimal_prec == ''){
            $aventura_decimal_prec = 2;
        }
        $aventura_price_label = number_format((float)$aventura_amount, $aventura_decimal_prec, $aventura_decimal_sep, $aventura_thousands_sep );
...
if ( ! function_exists( 'aventura_ajax_add_wishlist' ) ) {
    function aventura_ajax_add_wishlist() {
        $aventura_result_json = array( 'success' => 0, 'result' => array(), 'order_id' => 0 );
        if ( ! is_user_logged_in() ) {
...
//        $aventura_tour_type = aventura_metabox( 'aventura_tour_type','',$post_id,'' );
        ...
        $aventura_total_people = aventura_metabox( 'aventura_tour_total_people','',$post_id,'' );
        $aventura_total_people_booked = 0;
        $aventura_tour_max_adult        = isset( $aventura_options['aventura_tour_detail_max_adults'] ) ? $aventura_options['aventura_tour_detail_max_adults'] : '';
        $aventura_tour_max_kid        = isset( $aventura_options['aventura_tour_detail_max_kids'] ) ? $aventura_options['aventura_tour_detail_max_kids'] : '';
        $aventura_total_per_book = intval($aventura_tour_max_adult) + intval($aventura_tour_max_kid);
        global $wpdb;
        // Count Adults Booked
        $where = "1=1";
        $where .= " AND tour_id=" . $post_id;
        ...
        if($aventura_allow_manager_people == 1){
            ...
            return array('1',$aventura_total_per_book,'');
        }
    }
}
...
if ( ! function_exists( 'aventura_tour_check_availability_ajax' ) ) {
    function aventura_tour_check_availability_ajax() {
        ...
        $aventura_tour_time = isset($_POST['tour_time']) ?  $_POST['tour_time']: '';
...
//                'customer_address2',
            ...
            foreach ( $aventura_variables as $aventura_variable ) {
                $aventura_subject = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_subject );
                $aventura_description = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_description );
                $aventura_tour_name = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_tour_name );
                $aventura_booking_no = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_booking_no );
                $aventura_booking_date = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_booking_date );
                $aventura_booking_adults = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_booking_adults );
                $aventura_booking_kids = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_booking_kids );
                $aventura_name_combo = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_name_combo );
                $aventura_booking_total_price = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_booking_total_price );
                $aventura_order_payment_method = str_replace( "[" . $aventura_variable . "]", $aventura_variable, $aventura_order_payment_method );
            }
            ...
            if ( ! empty( $aventura_email_admin) && $aventura_email_admin == true ) {
                ...
                aventura_send_mail( $aventura_site_name,$aventura_customer_email, $aventura_admin_email, $aventura_admin_email, $aventura_subject, $aventura_description, $aventura_booking_no, $aventura_tour_name, $aventura_booking_date, $aventura_booking_time, $aventura_booking_adults, $aventura_booking_kids , $aventura_name_combo, $aventura_booking_total_price, $aventura_order_payment_method, $aventura_customer_first_name, $aventura_customer_last_name, $aventura_customer_phone, $aventura_customer_address, $aventura_customer_city, $aventura_customer_state, $aventura_customer_zip, $aventura_customer_country, $aventura_customer_order_notes, $aventura_admin_order_option, $aventura_admin_order_position );
            }
            return true;
        }
        return false;
...
if ( ! function_exists('aventura_send_mail') ) {
    function aventura_send_mail( $aventura_from_name,$aventura_customer_email, $aventura_from_address, $aventura_to_address, $aventura_subject, $aventura_description, $aventura_booking_no, $aventura_tour_name, $aventura_booking_date, $aventura_booking_time, $aventura_booking_adults, $aventura_booking_kids , $aventura_name_combo, $aventura_booking_total_price, $aventura_order_payment_method, $aventura_customer_first_name, $aventura_customer_last_name, $aventura_customer_phone, $aventura_customer_address, $aventura_customer_city, $aventura_customer_state, $aventura_customer_zip, $aventura_customer_country, $aventura_customer_order_notes , $aventura_order_option, $aventura_order_position ) {
        ...
        $aventura_to_address2 = '';
        if ($aventura_to_address == $aventura_from_address){
            $aventura_to_address2 = $aventura_customer_email;
        }else{
            $aventura_to_address2 = $aventura_to_address;
        }
...
if ( ! function_exists( 'aventura_tour_get_search_result' ) ) {
    function aventura_tour_get_search_result( $aventura_args ) {
        ...
        $aventura_order_date_default   = ( isset( $aventura_options['aventura_order_date'] ) && is_numeric($aventura_options['aventura_order_date']) )?$aventura_options['aventura_order_date']:0;
        $aventura_tour_expired   = ( isset( $aventura_options['aventura_tour_expired'] ) && is_numeric($aventura_options['aventura_tour_expired']) )?$aventura_options['aventura_order_date']:0;
...
//            var_dump(ICL_LANGUAGE_CODE);
        ...
        if ( ! empty( $aventura_categories_filter ) && trim( implode( '', $aventura_categories_filter ) ) != "" ) {
            $aventura_where .= " AND (( SELECT COUNT(1) FROM {$aventura_tbl_term_relationships} AS tr
					...
					WHERE tt1.taxonomy = 'tour-category' AND tt1.term_id IN (" . esc_sql( implode( ',', $aventura_categories_filter ) ) . ") AND tr.object_id = post_s1.ID ))";
        }
        ...
        if ( ! empty( $aventura_language_filter ) && trim( implode( '', $aventura_language_filter ) ) != "" ) {
            $aventura_where .= " AND (( SELECT COUNT(1) FROM {$aventura_tbl_term_relationships} AS tr1
					...
					WHERE tt1.taxonomy = 'tour-language' AND tt1.term_id IN (" . esc_sql( implode( ',', $aventura_language_filter ) ) . ") AND tr1.object_id = post_s1.ID ))";
        }
        ...
        if ( ! empty( $aventura_destination_filter ) && trim( implode( '', $aventura_destination_filter ) ) != "" ) {
            ...
            foreach ($aventura_destination_filter as $aventura_destination_sql){
                ...
                if($aventura_destination_count ==1){
                    $condition = 'AND(';
                }else{
                    $condition = 'OR';
                }
                $aventura_where .= " $condition (meta_destination_".esc_sql($aventura_destination_count).".meta_value IN (".esc_sql($aventura_destination_sql)."))";
        ...
        $aventura_sql .= "  INNER JOIN {$aventura_tbl_postmeta} AS meta_types ON post_s1.ID = meta_types.post_id AND meta_types.meta_key = 'aventura_tour_type' ";
        $aventura_where .=  " AND (meta_types.meta_value = 'special' OR meta_types.meta_value = 'daily' OR meta_types.meta_value = 'external')";
        $aventura_sql .= "  LEFT JOIN {$aventura_tbl_postmeta} AS meta_date ON post_s1.ID = meta_date.post_id AND meta_date.meta_key = 'aventura_tour_end_date' ";
        if($aventura_tour_expired ==0){
            //$aventura_where .=  " AND (meta_date.meta_value > '$current_time' OR meta_date.meta_value IS NULL)";
        }
        ...
        if ( ! empty( $aventura_order_by ) ) {
            ...
            if($aventura_order_date_default==1){
                $aventura_sql .= " ORDER BY post_s1.post_date " . $aventura_order;
            }
        }
...
function aventura_ajax_booking_form () {
    ...
    $aventura_tour_departure_time    	    = isset($_POST['departure_time']) ? $_POST['departure_time']: '';
        ...
        <div class="tz-tour-book-form">
            ...
            <form method="get" id="booking-form" action="<?php echo esc_url( aventura_get_tour_cart_page() ); ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="book-name">
                        <input name="first_name" value="" placeholder="<?php esc_html_e('First name','aventura') ?>" type="text" required>
                    </div>
                </div>
                <div class="form-group">
                    <label><?php esc_html_e('Last Name','aventura') ?></label>
                    <div class="book-name">
                        <input name="last_name" value="" placeholder="<?php esc_html_e('Last name','aventura') ?>" type="text" required>
                    </div>
                </div>
                <div class="form-group">
                    <label><?php esc_html_e('Email','aventura') ?></label>
                    <div class="book-email">
                        <input name="your_email" value="" placeholder="<?php esc_html_e('Email','aventura') ?>" type="text" required>
                    </div>
                </div>
                <div class="form-group">
                    <label><?php esc_html_e('Phone','aventura') ?></label>
                    <div class="book-phone">
                        <input name="your_phone" value="" placeholder="<?php esc_html_e('Phone','aventura') ?>" type="text" required>
                    </div>
                </div>
                ...
                <?php endif;?>
                <?php if( isset($aventura_adult_price) && $aventura_adult_price != '' ){ ?>
                    <div class="form-group <?php echo esc_html($aventura_class_has_combo); ?>">
                        ...
                        <div class="st_adults_children">
                            <div class="input-number-ticket">
                                <input class="input-number" name="number_adults" type="text" value="1" min="1" max="<?php echo esc_attr($aventura_max_adults); ?>" data-min="1" data-max="<?php echo esc_attr($aventura_max_adults); ?>"/>
                ...
                <?php }?>
                <?php if( isset($aventura_child_price) && $aventura_child_price != '' ){ ?>
                    <div class="form-group <?php echo esc_html($aventura_class_has_combo); ?>">
                        <label><?php esc_html_e('Children','aventura') ?></label>
                        <div class="st_adults_children">
                            <div class="input-number-ticket">
                                <input class="input-number" name="number_children" type="text" value="0" min="0" max="<?php echo esc_attr($aventura_max_kids); ?>" data-min="0" data-max="<?php echo esc_attr($aventura_max_kids); ?>"/>
                                <span class="input-number-decrement"><i class="fa fa-caret-left"></i></span><span class="input-number-increment"><i class="fa fa-caret-right"></i></span>
...
function aventura_review_lightbox () {
    ...
    $aventura_comments_number = aventura_parent_comment_counter($aventura_post_id);
            ...
            <li id="li-comment-<?php echo esc_attr($aventura_comment_ID); ?>">
                <div id="comment-<?php echo esc_attr($aventura_comment_ID); ?>" class="comments">
                    <div class="comment-meta comment-author vcard">
                        <?php echo get_avatar( $aventura_comment, 75 ); ?>
                    </div>
...
add_filter( 'woocommerce_product_get_tax_class', 'aventura_diff_rate_for_user', 1, 2 );

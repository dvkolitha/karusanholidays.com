...
if ( ! class_exists( 'Aventura_Tour_Order' ) ) {
    class Aventura_Tour_Order {
        ...
        public static function aventura_get_order_id( $booking_no, $pin_code ) {
            global $wpdb;
            $order_id = $wpdb->get_var( 'SELECT aventura_order.id FROM ' . $wpdb->prefix . 'aventura_order AS aventura_order WHERE aventura_order.booking_no="' . esc_sql( $booking_no ) . '" AND aventura_order.pin_code="' . esc_sql( $pin_code ) . '"' );
